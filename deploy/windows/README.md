...
已存在同名服务时安装包拒绝覆盖。卸载程序会停止并删除服务、程序文件和安装程序生成的桌面快捷方式，但有意保留 ProgramData 中的配置、CA 与 Agent 生成的身份/端口状态文件，供管理员决定是否备份或清理；这些文件含敏感材料。安装程序不会验证服务端在线或客户端认证成功，安装后需在服务端管理页确认客户端 Online，再检查 Agent 本机状态页。
...
脚本会先执行本地配置检查，再复制配置（旧配置另复制 CA）并安装自动启动且配置失败恢复策略的 Windows Service；没有配置文件不能安装。手动复制安装或卸载脚本时，须一并复制同目录的 `agent-monitor-shortcut.ps1`。调试时可直接用 `--config 'C:\ProgramData\RelayLink\Agent\agent.json'` 运行可执行文件。手动脚本与安装包都不会修改或删除已有的同名服务。
