...
public sealed class ControlSessionWorker(AgentConfiguration configuration, AgentStatus status, ILogger<ControlSessionWorker> logger) : BackgroundService
{
    private readonly Random random = new();
    private readonly X509Certificate2Collection? trustedRoots = configuration.UseTls ? LoadTrustedRoots(configuration.TrustedCaPemPath!) : null;
    ...
    private async Task RunSessionAsync(CancellationToken stoppingToken)
    {
        ...
        await using var peerSession = new PeerSessionCoordinator(configuration, identity, registration.SessionId, writer, CreateTransportStreamAsync, logger, sessionCancellation.Token);
        ...
        try
        {
            while (!sessionCancellation.IsCancellationRequested)
            {
                ...
                if (frame.Type == FrameType.Open)
                {
                    ...
                    _ = HandleOpenWithLimitAsync(open, registration.SessionId, activeConfiguration, writer, openLimit, sessionCancellation.Token);
                    continue;
                }
    ...
    private async Task HandleOpenWithLimitAsync(OpenMessage open, Guid sessionId, SessionConfiguration activeConfiguration, FrameWriter controlWriter, SemaphoreSlim openLimit, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await HandleOpenAsync(open, sessionId, activeConfiguration.Current, controlWriter, cancellationToken);
        }
        finally
        {
            openLimit.Release();
    ...
    private async Task HandleOpenAsync(OpenMessage open, Guid sessionId, (string Version, ClientConfigSnapshot Config) activeConfiguration, FrameWriter controlWriter, CancellationToken sessionCancellation)
    {
        ...
        try
        {
            ...
            await dataClient.ConnectAsync(configuration.ServerHost, configuration.ServerPort, deadline.Token);
            await using Stream tls = await CreateTransportStreamAsync(dataClient, deadline.Token);
            var reader = new FrameReader(tls);
            var writer = new FrameWriter(tls);
            ...
            await writer.WriteAsync(new Frame(FrameType.TargetReady, JsonProtocolSerializer.Serialize(new TargetReadyMessage(open.ConnectionId, duration))), deadline.Token);
            var start = await reader.ReadAsync(ProtocolConstants.MaxControlPayloadLength, deadline.Token);
            if (start?.Type != FrameType.Start || JsonProtocolSerializer.Deserialize<StartMessage>(start.Payload.Span).ConnectionId != open.ConnectionId)
            {
                throw new ProtocolException("Server did not start data forwarding.");
            }

            using var relayCancellation = CancellationTokenSource.CreateLinkedTokenSource(sessionCancellation);
            var toServer = RelayPump.CopySocketToFramesAsync(target.Client, writer, relayCancellation.Token);
            var toTarget = RelayPump.CopyFramesToSocketAsync(reader, target.Client, relayCancellation.Token);
        ...
        {
            logger.LogDebug(exception, "Target connection failed for {ChannelId}.", open.ChannelId);
        ...
        {
            logger.LogDebug(exception, "Data tunnel failed for {ChannelId}.", open.ChannelId);
    ...
    private static X509Certificate2Collection LoadTrustedRoots(string path)
    {
