...
- 控制连接与按业务连接建立的数据隧道共用服务端接入端口。普通代理可按部署配置使用 TLS；客户端安全互访要求外层 TLS，并在两端 Agent 之间建立内层 TLS。
- 服务端管理页面支持匿名只读查看状态、通道和流量历史；管理员登录后可管理客户端、通道及访问映射。
- Agent 提供本机只读状态页，展示通道和可用的访问方地址。Windows Agent 可作为开机自动启动的 Windows Service 运行。

## 组成与边界

服务端支持 Linux 和 Windows，Agent 面向 Windows。后端使用 C# / .NET，管理前端使用 React 和 TypeScript。服务端接入端口可对 Agent 开放；业务代理端口及管理页面应限定在受信任内网，网络访问控制由部署环境的安全组或防火墙承担。
