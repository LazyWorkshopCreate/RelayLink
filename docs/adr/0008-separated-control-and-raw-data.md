...
- 控制连接继续按 `tunnel.tlsEnabled` / Agent `useTls` 使用外层 TLS 与服务端证书验证。客户端安全互访仍要求控制 TLS，以保护映射访问密钥的下发；互访业务流继续使用两端 Agent 的内层 TLS、固定证书指纹和访问证明。互访连接在数据端口上的外层传输不使用 TLS，但当前仍保留加密记录的帧化中继及应用半关闭封装。
...
- 完全取消互访内层 TLS：违反既有授权与端到端保密要求，不采用。
